...
import { PixelFileItem, PixelConfig } from '@/lib/pixelTypes';
import { formatBytes } from '@/lib/renameEngine';
import { ComparisonSlider } from '@/components/compressor/ComparisonSlider';

interface SinglePixelViewProps {
  item: PixelFileItem;
  config: PixelConfig;
  onResize: () => void;
  onDownload: () => void;
  onRemove: () => void;
  onUpdateConfig: <K extends keyof PixelConfig>(key: K, value: PixelConfig[K]) => void;
  isProcessing: boolean;
}

export function SinglePixelView({
  item,
  config,
  onResize,
  onDownload,
  ...
  // Calculate resolution & pixel delta
  ...
  return (
    <div className="bg-white dark:bg-[#121620] border border-zinc-200 dark:border-zinc-800 rounded-3xl p-5 sm:p-7 shadow-xs space-y-6">
      ...
      {/* Safety Warning if image is massive */}
      {item.isExcessiveSize && (
        <div className="flex items-start gap-2.5 p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 text-amber-800 dark:text-amber-300 text-xs">
          <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0 text-amber-600" />
          <div>
            <span className="font-semibold">Memory Notice:</span> {item.safetyWarning}
          </div>
        </div>
      )}

      {/* Live Dimension Preview Banner */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {/* Original vs Target Dimensions */}
        <div className="p-3 bg-zinc-50 dark:bg-[#161b26] rounded-2xl border border-zinc-200/70 dark:border-zinc-800">
          ...
          >
            {pixelChangePct > 0 ? `+${pixelChangePct}% area` : `${pixelChangePct}% area`}
          </span>
        </div>
        ...
        <div className="p-3 bg-zinc-50 dark:bg-[#161b26] rounded-2xl border border-zinc-200/70 dark:border-zinc-800">
          ...
          <p className="text-sm font-bold text-zinc-800 dark:text-zinc-100 mt-1 capitalize">
            {config.mode}
          </p>
          <span className="text-[10px] text-zinc-400 font-medium">
        ...
        <div className="p-3 bg-zinc-50 dark:bg-[#161b26] rounded-2xl border border-zinc-200/70 dark:border-zinc-800">
          ...
          {sizeDiffPct !== null ? (
            ...
            >
              {sizeDiffPct < 0 ? `${Math.abs(sizeDiffPct)}% smaller` : `+${sizeDiffPct}% larger`}
            </span>
          ) : (
            <span className="text-[10px] text-zinc-400">Calculated upon resize</span>
          )}
        </div>
      </div>
        ...
        >
          {isProcessing ? (
            <>
              ...
              <span>Resizing Image in Browser...</span>
            </>
          ) : (
            <>
              <Sparkles className="w-4 h-4" />
              <span>{hasOutput ? 'Re-Apply Pixel Dimensions' : 'Resize Image Now'}</span>
            </>
          )}
        </button>
