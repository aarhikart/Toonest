...
interface BulkPixelViewProps {
  ...
  onSort: (type: 'name' | 'width' | 'height' | 'size' | 'format') => void;
  onPreviewItem: (item: PixelFileItem) => void;
  onOpenCustomSettings: (item: PixelFileItem) => void;
  isProcessing: boolean;
  progressPercent: number;
  completedCount: number;
  failedCount: number;
  stats: PixelBatchStats;
}

export function BulkPixelView({
  items,
  config,
  onResizeAll,
  onCancelResize,
        ...
        <div className="p-4 rounded-2xl bg-[#5722AF]/5 dark:bg-[#5722AF]/10 border border-[#5722AF]/20 space-y-3 animate-in fade-in">
          <div className="flex items-center justify-between text-xs font-bold">
            <span className="text-[#5722AF] dark:text-[#9B6BE8] flex items-center gap-2">
              ...
              <span>
                Processing {completedCount + failedCount + 1} of {items.length} images...
              </span>
            </span>
        ...
        <div className="p-5 rounded-2xl bg-emerald-50/90 dark:bg-emerald-950/30 border border-emerald-200/80 dark:border-emerald-800/50 space-y-4 animate-in fade-in">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300 font-bold text-sm">
              ...
              <span>
                Images resized successfully — {stats.completedFiles} of {items.length} processed
              </span>
            </div>
            {failedCount > 0 && (
              <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            <div className="bg-white/80 dark:bg-[#131722]/80 p-3 rounded-xl border border-emerald-200/60 dark:border-emerald-900/40">
              <div className="text-zinc-400 font-medium">New Total Size</div>
              <div className="font-bold text-[#5722AF] dark:text-[#9B6BE8] mt-0.5">
                {formatBytes(stats.outputTotalBytes)}
              </div>
            </div>
            <div className="bg-white/80 dark:bg-[#131722]/80 p-3 rounded-xl border border-emerald-200/60 dark:border-emerald-900/40">
              <div className="text-zinc-400 font-medium">Difference</div>
              <div
        ...
        <table className="w-full text-left border-collapse hidden md:table">
          <thead>
            <tr className="border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/80 dark:bg-[#181d2a] text-[11px] font-bold uppercase tracking-wider text-zinc-500">
              ...
              <th className="py-3 px-3 w-36">Target Dim</th>
              <th className="py-3 px-3 w-28">Original Size</th>
              <th className="py-3 px-3 w-28">New Size</th>
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800 text-xs">
            {items.map((item, idx) => {
              ...
              const hasCustom = Boolean(item.customOverride);
              ...
              return (
                ...
                >
                  ...
                  {/* Filename & Ratio */}
                  <td className="py-3 px-3 font-medium max-w-[190px]">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      {hasCustom && (
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-[#5722AF]/10 text-[#5722AF] dark:text-[#9B6BE8]">
                          Custom
                        </span>
                      )}
                    </div>
                  ...
                  {/* Actions (Custom settings, Download, Remove) */}
                  <td className="py-3 px-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        type="button"
                        onClick={() => onOpenCustomSettings(item)}
                        className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                          hasCustom
                            ? 'text-[#5722AF] bg-[#5722AF]/10 hover:bg-[#5722AF]/20'
                            : 'text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800'
                        }`}
                        title="Configure custom settings for this image"
                      >
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="overflow-hidden">
                    <div className="flex items-center gap-1.5">
                      <p className="text-xs font-bold text-zinc-900 dark:text-white truncate max-w-[140px]">
                        {item.originalName}
                      </p>
                      {hasCustom && (
                        <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-[#5722AF]/10 text-[#5722AF]">
                          Custom
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-1">
                  <button
                    type="button"
                    onClick={() => onOpenCustomSettings(item)}
                    className="p-1.5 rounded-lg text-zinc-400 hover:text-[#5722AF] cursor-pointer"
              ...
              <div className="flex items-center justify-between p-2 rounded-xl bg-white dark:bg-[#11151f] border border-zinc-200/60 dark:border-zinc-800 text-xs">
                ...
                <div>
                  <span className="text-[10px] text-zinc-400 block">Target Size</span>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {isProcessing ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin" />
                <span>Processing Batch ({completedCount}/{items.length})...</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4" />
                <span>
                  {anyCompleted ? 'Re-Apply to All Images' : `Resize All (${items.length} Images)`}
                </span>
              </>
            )}
          </button>
