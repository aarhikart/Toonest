...
import { ShadowConfig } from '@/lib/boxShadow/shadowTypes';
import { exportShadowPreviewToCanvas } from '@/lib/boxShadow/shadowEngine';
...
export function ImageExportModal({ isOpen, onClose, config }: ImageExportModalProps) {
  ...
  const defaultFilename = `box-shadow-${(config.name || 'preview')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '-')
    .replace(/-+/g, '-')}`;
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      const blob = await exportShadowPreviewToCanvas(
        config,
        width,
        height,
        format,
      ...
      <div className="bg-white dark:bg-[#1E1E2A] rounded-2xl max-w-lg w-full border border-gray-200 dark:border-[#2D2D3F] shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-[#2D2D3F]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-gray-900 dark:text-white">Export Preview as Image</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">Download high-res raster snapshot of current shadow</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[75vh]">
                ...
                <button
                  ...
                  className={`text-left p-2.5 rounded-lg border text-xs font-medium transition-all ${
                    width === preset.w && height === preset.h
                      ? 'border-[#5722AF] bg-[#5722AF]/5 text-[#5722AF] dark:bg-[#5722AF]/15'
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                max="3840"
                value={width}
                onChange={(e) => setWidth(Math.max(200, Math.min(3840, Number(e.target.value) || 200)))}
            ...
            <div>
              ...
              <input
                ...
                max="3840"
                value={height}
                onChange={(e) => setHeight(Math.max(200, Math.min(3840, Number(e.target.value) || 200)))}
                className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-[#2D2D3F] bg-white dark:bg-[#12121A] text-sm text-gray-900 dark:text-white focus:outline-hidden focus:border-[#5722AF]"
              />
            </div>
          </div>
          ...
          {format === 'image/png' && (
            <div className="flex items-center justify-between pt-1">
              <div>
                <div className="text-sm font-medium text-gray-800 dark:text-gray-200">Transparent Background</div>
                <div className="text-xs text-gray-500 dark:text-gray-400">Exclude preview canvas background color</div>
              </div>
              <input
                type="checkbox"
          ...
          {format === 'image/jpeg' && (
            <div className="space-y-1.5 pt-1">
              <div className="flex justify-between text-xs">
                <span className="font-medium text-gray-700 dark:text-gray-300">JPEG Quality</span>
