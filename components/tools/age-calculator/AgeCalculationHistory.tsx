...
import { AgeHistoryItem } from '@/lib/age/types';

interface AgeCalculationHistoryProps {
  isOpen: boolean;
  onClose: () => void;
  items: AgeHistoryItem[];
  onReuse: (item: AgeHistoryItem) => void;
  onDelete: (id: string) => void;
  onClear: () => void;
}

export const AgeCalculationHistory: React.FC<AgeCalculationHistoryProps> = ({
  isOpen,
  onClose,
  items,
  onReuse,
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 max-w-xs">
                  Your calculated ages and dates will automatically be remembered here for easy recall.
                </p>
              </div>
            </div>
          ) : (
            items.map((item) => (
              ...
              >
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  As of: <span className="font-medium text-slate-700 dark:text-slate-300">{item.targetFormatted}</span>
                </div>
